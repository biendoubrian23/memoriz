...
function PageContent({
  ...
  // ── Magazine freeform layout: render a read-only preview ──
                ...
                {el.element_type === "shape" && (
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: (styles.shapeType as string) === "circle" ? "50%" : (styles.borderRadius != null ? `${styles.borderRadius}%` : undefined),
                    }}
                  />
                )}
                {el.element_type === "sticker" && (
                  <div className="w-full h-full flex items-center justify-center text-[min(100%,100%)]">
                    <span style={{ fontSize: "min(100%, 100cqh)" }}>{el.content}</span>
                  </div>
                )}
              </div>
            );
